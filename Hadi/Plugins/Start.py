...
from Hadi import ASSIDS, BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from Hadi import boottime as bot_start_time
from Hadi import db, random_assistant
from Hadi.Core.PyTgCalls import Hadi
from Hadi.Database import (add_nonadmin_chat, add_served_chat,
...
from Hadi.Decorators.admins import ActualAdminCB
from Hadi.Decorators.permission import PermissionCheck
from Hadi.Inline import (custommarkup, dashmarkup, setting_markup,
                          setting_markup2, start_pannel, usermarkup, volmarkup)
from Hadi.Utilities.assistant import get_assistant_details
from Hadi.Utilities.ping import get_readable_time
        ...
        try:
            ...
            if member.id in OWNER_ID:
                return await message.reply_text(
                    f"Ehh Owner[{member.mention}] has just joined your chat."
                )
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "LV":
        ...
        volume = 25
        try:
            await Hadi.pytgcalls.change_volume_call(c_id, volume)
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"Hadi Disk Usage: {diske}%", show_alert=True
        )
